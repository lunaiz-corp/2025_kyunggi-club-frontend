...
export default function NoticeListTable() {
  ...
  return (
    <div className="flex flex-col gap-24">
      <table className="w-full border-collapse border-spacing-x-12 border-b border-gray-700">
        <thead className="h-10 border-b border-gray-500 align-top">
          <tr>
            <th scope="col" className="w-[18px] pr-7">
              <Checkbox
                id="toggle-all"
                checked={
                ...
                onChange={e => {
                  ...
                  if (checked) {
                    setCheckedNotices([...checkedNotices, ...[1, 2]])
                  } else {
                    setCheckedNotices([])
                  }
                }}
